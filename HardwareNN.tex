...
\begin{document}
    ...
    %In \cite{muthuramalingam2008neural} an AF $f(x) = \frac{1}{1+e^{-x}}$ is computed by spliting the
    ...
    Digital weight storage is free of noise and therefor produces the same output across different devices.
    ...
    A processor that manages the entire network is often times not required for normal classification and therefor not present. \\
    ...
    Hardware ANNs seem to be a promising alternative to classic processor architectures.
    ...
    the large scale parallelism of these processors allows for faster simulation of neural networks than on classical processors.\\
    Due to the multi-purpose layout of these processors, they can simulate a variety of network topologies and are also capable of training a network without any additional hardware.
